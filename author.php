<?php
session_start();
if ((!isset($_SESSION['logged'])) || (!isset($_GET['id_author']))) {
    ...
    <section id="category-quotes">
        <div class="wrapper">
            <div class="box-info">
                <div class="box-left">
                    <?php
                    echo "<h3 class='info-two'>Name: <span class='gradient'>" . $_SESSION['author_name'] . "</span></h3>";
                    if (!empty($_SESSION['author_surname'])) {
                        echo "<h3 class='info-two'>Surname: <span class='gradient'>" . $_SESSION['author_surname'] . "</span></h3>";
                    }
                    if (!empty($bornDateGood)) {
                        echo "<h4 class='info-two'>Date of born: <span class='gradient'>" . $_SESSION['author_date_born'] . "</span></h4>";
                    }
                    if (!empty($deathDateGood)) {
                        echo "<h4 class='info-two'>Date of death: <span class='gradient'>" . $_SESSION['author_date_death'] . "</span></h4>";
                    }
                    echo "<h3 class='info-two'>Number of quotes on website: <span class='gradient'>" . $_SESSION['quantity_of_quotes'] . "</span></h3>";
                    ?>
                </div>
                <div class="box-right">
                    <?php
        ...
        <div class="quote-box">
            ...
            try {
                ...
                if ($link->connect_errno != 0) {
                    ...
                    $result = $link->query("SELECT quotes.*, quotes.id AS 'quote_id', categories.name AS 'category_name', authors.*, authors.id AS 'author_id' FROM quotes INNER JOIN authors ON quotes.author_id=authors.id INNER JOIN categories ON quotes.categories_id=categories.id WHERE quotes.categories_id=6 ORDER BY RAND() LIMIT 1");
                    if (!$result) {
                        throw new Exception($link->error);
                    }
                    $how_many_rand_quotes = $result->num_rows;
