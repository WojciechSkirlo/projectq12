...
try {
    ...
    if ($link->connect_errno != 0) {
        ...
        $result = $link->query("SELECT users.user, users.role, users.img_user FROM users WHERE users.id='$user_id'");
        if (!$result) {
        ...
        $result = $link->query("SELECT COUNT(quotes.id) AS 'added_quote' FROM quotes INNER JOIN users ON users.id=quotes.user_id WHERE users.id='$user_id'");
        if (!$result) {
            throw new Exception($link->error);
        }
        $how_many = $result->num_rows;
...
<head>
    ...
    <title>
        <?php echo "ProjectQ12 | User - " . $user_name ?>
    </title>
...
<body>
    ...
    <section id="home-category">
        <div class="wrapper">
            <h3 class="category">User <span class="author gradient"> -
                    <?php
                    echo $user_name;
                    ?>
                </span>
        ...
        <div class="wrapper">
            <div class="user-section">
                <div class="user-section-left">
                    <?php
                    echo '<h3 class="info-two">User: <span class="gradient">' . $user_name . '</span></h3>';
                    echo '<h3 class="info-two">Role: <span class="gradient">' . $user_role . '</span></h3>';
                    echo '<h3 class="info-two">Number of added quotes: <span class="gradient">' . $added_quote . '</span></h3>';
                    ?>
                </div>
                <div class="user-section-right">
                    <?php
                    echo '<img src="' . $user_img . '" alt="User: ' . $user_name . '"/>';
                    ?>
                </div>
            </div>
        </div>
        <div class="wrapper">
            ...
            try {
                ...
                if ($link->connect_errno != 0) {
                    ...
                    $result = $link->query("SELECT id FROM quotes WHERE user_id='$user_id'");
    ...
    if ($number_of_pages > 1) {
        ...
        for ($page = 1; $page <= $number_of_pages; $page++) {
            if ($number_of_pages != 1) {
                echo '<div class="pagination-page"><a href="user.php?id_user=' . $user_id . '&page=' . $page . '" class="';
                if ($page == $_GET['page']) {
                    echo "active";
                }
                echo '">' . $page . '</a></div>';
